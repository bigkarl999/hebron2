        ...
        return (
          <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            <div className="space-y-2">
              ...
              <Input id="full_name" placeholder="Enter your full name" value={formData.full_name} onChange={(e) => setFormData({ ...formData, full_name: e.target.value })} className="input-soft h-12 text-base" data-testid="input-full-name" />
            </div>
          </motion.div>
        );
            ...
            <RadioGroup value={formData.role} onValueChange={(value) => setFormData({ ...formData, role: value, date: "" })} className="grid gap-4 md:grid-cols-2">
              <Label htmlFor="prayer" className={`cursor-pointer rounded-xl border-2 p-6 transition-all ${formData.role === "Prayer" ? "border-blue-500 bg-blue-50" : "border-orange-100 hover:border-orange-200 hover:bg-orange-50/50"}`}>
                ...
                <div className="flex flex-col items-center text-center">
                  <div className="mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-blue-100"><HandHeart className="h-8 w-8 text-blue-600" /></div>
                  <span className="font-['Playfair_Display'] text-xl font-semibold">Lead Prayer</span>
                  <span className="mt-1 text-sm text-muted-foreground">Guide the congregation in prayer</span>
                </div>
              </Label>
              <Label htmlFor="worship" className={`cursor-pointer rounded-xl border-2 p-6 transition-all ${formData.role === "Worship" ? "border-purple-500 bg-purple-50" : "border-orange-100 hover:border-orange-200 hover:bg-orange-50/50"}`}>
                ...
                <div className="flex flex-col items-center text-center">
                  <div className="mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-purple-100"><Music className="h-8 w-8 text-purple-600" /></div>
                  <span className="font-['Playfair_Display'] text-xl font-semibold">Lead Worship</span>
                  <span className="mt-1 text-sm text-muted-foreground">Lead the congregation in praise</span>
                </div>
              </Label>
            </RadioGroup>
          </motion.div>
        ...
        return (
          <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
            <div className="rounded-xl bg-gradient-to-r from-orange-50 to-amber-50 p-4">
              <div className="flex items-center gap-3"><Clock className="h-5 w-5 text-orange-600" /><p className="text-sm font-medium text-foreground">All online meetings are from 8:00 PM to 9:00 PM UK time.</p></div>
            </div>
            ...
            <div className="flex justify-center">
              <Calendar mode="single" selected={formData.date ? new Date(formData.date) : undefined} onSelect={(date) => date && setFormData({ ...formData, date: format(date, "yyyy-MM-dd") })} disabled={disabledDays} className="rounded-xl border border-orange-100 p-3" data-testid="booking-calendar" />
            </div>
      ...
      case 4:
        return (
          <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
            <div className="rounded-xl border-2 border-green-200 bg-green-50/70 p-5">
              <div className="mb-3 flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-800/80">Receive your booking confirmation and same-day reminder on WhatsApp.</p>
                </div>
              </div>
            ...
            <div>
              <Button type="button" variant="outline" onClick={() => setShowEmail((value) => !value)} className="w-full justify-start gap-2" data-testid="toggle-email">
                <Mail className="h-4 w-4" />
                {showEmail ? "Hide email option" : "Prefer email? Add email address"}
              </Button>
              {showEmail && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-3 rounded-xl border border-border p-4">
                  ...
                  <div className="relative mt-2">
                    ...
                    <Input id="email" type="email" placeholder="your@email.com" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="input-soft h-12 pl-10 text-base" data-testid="input-email" />
                  </div>
                  <p className="mt-2 text-xs text-muted-foreground">If entered, you will also receive the confirmation and reminder by email.</p>
                </motion.div>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea id="notes" placeholder="Any special requests or comments..." value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="input-soft min-h-[100px]" data-testid="input-notes" />
            </div>

            <div className="rounded-xl border border-orange-200 bg-orange-50/50 p-6">
              <h4 className="mb-4 font-['Playfair_Display'] text-lg font-semibold">Booking Summary</h4>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between gap-4"><span className="text-muted-foreground">Name:</span><span className="text-right font-medium">{formData.full_name}</span></div>
                <div className="flex justify-between gap-4"><span className="text-muted-foreground">Role:</span><span className="text-right font-medium">Lead {formData.role}</span></div>
                <div className="flex justify-between gap-4"><span className="text-muted-foreground">Date:</span><span className="text-right font-medium">{formData.date && format(new Date(formData.date), "EEEE, MMMM d, yyyy")}</span></div>
                <div className="flex justify-between gap-4"><span className="text-muted-foreground">Time:</span><span className="text-right font-medium">8:00 PM - 9:00 PM UK</span></div>
                {formData.phone_number && <div className="flex justify-between gap-4"><span className="text-muted-foreground">WhatsApp:</span><span className="text-right font-medium">{formData.phone_number} ✓</span></div>}
                {formData.email && <div className="flex justify-between gap-4"><span className="text-muted-foreground">Email:</span><span className="text-right font-medium">{formData.email}</span></div>}
              </div>
            </div>
          </motion.div>
        );
      ...
      <div className="container mx-auto px-4 py-8 md:py-12">
        <div className="mx-auto max-w-2xl">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8 text-center">
            <h1 className="font-['Playfair_Display'] text-3xl font-bold text-foreground md:text-4xl">Book Your Slot</h1>
            <p className="mt-2 text-muted-foreground">Sign up to serve in our online meetings</p>
          </motion.div>
              ...
              {steps.map((step, index) => (
                <div key={step.id} className="flex flex-1 items-center">
                  <div className="flex flex-col items-center">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-full transition-all ${currentStep >= step.id ? "bg-gradient-to-r from-orange-500 to-red-600 text-white" : "bg-orange-100 text-muted-foreground"}`}>
                  ...
                  {index < steps.length - 1 && <div className={`mx-2 h-1 flex-1 rounded-full transition-all ${currentStep > step.id ? "bg-orange-500" : "bg-orange-100"}`} />}
                </div>
              ))}
            </div>
          </div>
          ...
          <Card className="card-warm" data-testid="booking-form-card">
            <CardHeader><CardTitle className="font-['Playfair_Display'] text-xl">Step {currentStep}: {steps[currentStep - 1].title}</CardTitle></CardHeader>
            <CardContent>
              <AnimatePresence mode="wait">{renderStepContent()}</AnimatePresence>
              <div className="mt-8 flex justify-between gap-4">
                <Button variant="outline" onClick={handleBack} disabled={currentStep === 1} className="gap-2" data-testid="btn-back"><ArrowLeft className="h-4 w-4" />Back</Button>
                {currentStep < 4 ? (
